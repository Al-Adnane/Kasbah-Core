...
class KasbahAgentControl {
  ...
  async govern(request) {
    ...
    let sentinelResult;
    try {
      // ── Bridge: sentinelCheck() is the real API on KasbahSentinel.
      // sentinel.evaluate() never existed — this was the root of all
      // DENY-on-safe-input failures. Normalise the request, call the
      // correct method, then map the return back to {overallRisk}.
  ...
  _verifyTOCTOU(snapshot, request) {
    ...
    // Verify request hasn't been mutated since snapshot
   ...
   * Handles sdk_call, tool_use, agent_message, llm_call, raw strings.
   * Extracts all threat surface fields from diverse request shapes.
   */
  _normaliseSentinelInput(request) {
    ...
    if (Array.isArray(request.args)) {
      request.args.forEach(a => {
        if (Array.isArray(a.messages)) a.messages.forEach(m => m.content && textParts.push(m.content));
        if (typeof a === 'string') textParts.push(a);
      });
    }
    if (request.input) {
      textParts.push(typeof request.input === 'string' ? request.input : JSON.stringify(request.input));
    }
    const content = textParts.filter(Boolean).join(' ').trim();

    let verb = 'check', target = '*';
