...
echo "== Restore + patch entrypoint_api.sh (literal, no host expansion) =="

docker exec -i "$CN" sh -lc "cat > /tmp/patch_entrypoint.sh <<'SH'
set -e
...
echo
echo '== Final entrypoint_api.sh (1..90) =='
nl -ba /app/scripts/entrypoint_api.sh | sed -n '1,90p'
SH
sh /tmp/patch_entrypoint.sh"
